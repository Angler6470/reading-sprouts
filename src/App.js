  ...
  return (
    <div className={`${currentTheme.bg} flex flex-col items-center p-3 pt-[calc(env(safe-area-inset-top)+0.75rem)] pb-[calc(env(safe-area-inset-bottom)+0.75rem)] font-sans ${currentTheme.textColor || 'text-stone-800'} overflow-hidden relative transition-colors duration-500`} style={{ minHeight: 'calc(var(--app-vh, 1vh) * 100)' }}>
        ...
        <div className="bg-white/50 rounded-xl p-1.5 shadow-sm flex items-center justify-center">
          <div className="flex flex-col items-center gap-1">
            <div className="flex items-center gap-2">
              ...
              <h1 className={`text-sm font-black ${currentTheme.accent} leading-none`} style={{ fontFamily: '"Bubblegum Sans", cursive' }}>Reading Sprouts</h1>
            </div>
      ...
      <main className="flex-1 min-h-0 flex flex-col items-center justify-center w-full max-w-md gap-3 py-1 overflow-hidden">
        ...
        <div className={`bg-white rounded-xl p-3 shadow-md border-b-2 ${currentTheme.problemBorder} w-full shrink-0 transition-transform ${feedback.type === 'error' ? 'animate-shake' : ''}`}>
        ...
        <div className="flex gap-2 shrink-0">{problem.options.map((option, index) => (
            <button
              ...
              className={`rounded-full text-[13px] font-bold text-white shadow-md transform transition-all duration-500 ease-in-out active:scale-95 ${currentTheme.btnColors[index]} ${hintedOptionIndex === index ? 'opacity-0 scale-0 w-0 h-0 m-0 pointer-events-none p-0 border-0' : 'w-11 h-11'}`}
            >
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-t-xl px-2 py-1 shadow-md">
          <div className="flex items-center gap-2">
            <div className="flex-1 h-2 bg-stone-100 rounded-full overflow-hidden border border-stone-200">
              <div className={`h-full rounded-full transition-all duration-700 ease-out bg-gradient-to-r ${currentTheme.progressGradient}`} style={{ width: `${seeds * 10}%` }}></div>
            </div>
